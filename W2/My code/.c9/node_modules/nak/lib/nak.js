...
var walkdir = require('../lib/walkdir').walkdir;
var PathFilter = require('../lib/path-filter');
...
if (process.platform == "win32" && (fpath[0] == "/" || !fpath)) {
  options.startPaths = options.startPaths && options.startPaths.map(function(p) {
    return p.replace(/^\//, "");
  });
...
if (options.color) {
  var queryString = String(options.query);
